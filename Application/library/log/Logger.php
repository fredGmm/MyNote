...
class Logger
{
    ...
    public function __construct(Config $config, array $options = [])
    {
        ...
        $this->isCli = (php_sapi_name() === 'cli'); //是否命令行模式
        ...
        if (is_string($this->options['logLevelThreshold'])) {
            $this->options['logLevelThreshold'] = isset($this->logLevels[$this->options['logLevelThreshold']]) ? $this->logLevels[$this->options['logLevelThreshold']] : Config::INFO;
        }
    ...
    protected function formatMessage($level, $message, $args)
    {
        ...
        $this->logLineKeywords['time'] = date('Y-m-d H:i:s', $timestamp) . substr((string)$microSeconds, 1, 4);
